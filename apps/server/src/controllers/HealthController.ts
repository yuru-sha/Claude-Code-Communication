...
import { ACTIVITY_DETECTION_CONFIG, SystemHealth } from '@claude-communication/types';
...
// Using SystemHealth interface from types package - see @claude-communication/types
...
export const performHealthCheck = async (): Promise<SystemHealth> => {
  try {
    ...
    // Log only critical health issues
    return health;
...
export const performAutoRecovery = async (): Promise<boolean> => {
  if (isRecoveryInProgress) {
    // Auto recovery already in progress
    return false;
  }
...
export const updateCheckInterval = (hasActiveAgents: boolean): void => {
  ...
  if (newInterval !== currentCheckInterval) {
    currentCheckInterval = newInterval;
      // Health check interval adjusted
