...
import { useSocket } from '@claude-communication/ui';
...
import { TaskPipeline } from '@claude-communication/ui';
import { DashboardHeader } from '@claude-communication/ui';
import { Task, Agent, SystemHealth } from '@claude-communication/types';
...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on('auto-recovery-status', (data: any) => {
      ...
      setTimeout(() => setAutoRecoveryStatus(null), 5000);
    });

    socket.on('auto-recovery-failed', (data: any) => {
      setAutoRecoveryStatus(`❌ 自動復旧失敗: ${data.message}`);
      setTimeout(() => setAutoRecoveryStatus(null), 10000);
    });
